...
public class UIControllerStub  {
	...
	private LogicStub mainLogic;
	private CommandTypeStub prevCommandType = null;
	...
	private void executeCommand(String userCommand) {
		...
		/*
		 * TODO: Commands are temporary and for skeletal purpose some will be
		 * removed or changed when UI has been updated
		 */
		CommandTypeStub currCommandType = mainLogic.getCmdType(userCommand);
		String commandContents = mainLogic.getCmdContent(userCommand);

		switch (currCommandType) {
		case ADD:
			/*
			 * Done addedTask = mainLogic.getTask(userCommand); assert addedTask
			 * != null; showAdd(addedTask);
			 */
		...
		case DISPLAY:
			/*
			 * TODO: Enable display command to fit in parameters such that it
			 * will be able to display <all> <floating> <timed> <deadline>
			 */
			break;
		case DELETE:
			/*
			 ...
			 */
			String taskName = mainLogic.deleteTask(Integer
					.parseInt(commandContents) - ARRAY_DELETE_OFFSET);
			showDelete(taskName);
			break;
		case LOAD:
	...
	private void showUndo(Done task) {
		if (prevCommandType != null) {
			if (task != null) {
				...
				Notifications
						.create()
						.text(String.format(SHOWUNDO_SUCCESS_MESSAGE,
								prevCommandType, undoneTitle))
						.showInformation();
			} else {
				Notifications
						.create()
						.text(String.format(SHOWUNDO_SUCCESS_MESSAGE,
								prevCommandType, EMPTY_STRING))
						.showInformation();
			}
	...
	private void showInvalidCommand() {
		Notifications.create().text(SHOWINVALIDCOMMAND_ERROR_MESSAGE)
				.showError();
	}
	...
	private void showDelete(String task) {
		if (task != null) {
			...
			Notifications.create()
					.text(String.format(SHOWDELETE_SUCCESS_MESSAGE, task))
					.showInformation();
			display();
		} else {
	...
	private void showAdd(String task) {
		...
		Notifications.create()
				.text(String.format(SHOWADD_SUCCESS_MESSAGE, task))
				.showInformation();
		display();
	}
	...
	private void display() {
		...
		ObservableList<Done> tableTasks = FXCollections
				.observableArrayList(tasks);
		tableViewTasks.setItems(tableTasks);
		tableViewTasks.getColumns().get(0)
