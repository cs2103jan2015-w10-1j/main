...
public class LogicStub {
	...
	public LogicStub() {
		...
		cmdParser = new CommandParserStub();
	}
	...
	public CommandTypeStub getCmdType(String usercommand) {
		...
		try {
			...
			for (int i = 0; i < tasks.size(); i++) {
				...
				if (temp.contains(searchString.toLowerCase())) {
					flag = 1;
					searchVector.add(new DoneFloatingTask((i + 1) + ". "
							+ tasks.get(i)));
				}
			}
	...
	// private service method useful to implement setReminder(), markdone() and
	// other methods.
